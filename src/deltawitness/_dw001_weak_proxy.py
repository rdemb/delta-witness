"""Fixed weak-proxy-oracle fixture for the DW-001 Gate 1 boundary.

The fixture is an owned-synthetic agent-workflow surrogate. Its declared test
is a genuine fail-to-pass assertion over a weak proxy property. The candidate
implements the intended administrator-only behavior, while a separately fixed
mutant preserves the proxy property and violates viewer denial.

No caller-provided source, test, prompt, or mutant bytes are accepted.
"""
...
BASE_CODE = """def is_admin(user):
    return user.get(\"role\")
"""

BASE_TESTS = """import sys
import unittest
...
CANDIDATE_CODE = """def is_admin(user):
    return user.get(\"role\") == \"admin\"
"""

CANDIDATE_TESTS = """import sys
import unittest
...
MUTANT_CODE = """def is_admin(user):
    return bool(user.get(\"role\"))
"""

HIDDEN_CLAIM_TESTS = """import sys
import unittest
...
def expected_states(observer: str) -> list[dict[str, object]]:
    if observer == "exit-code-v1":
        ...
        causes = ("none", "assertion_failure", "none", "none")
    else:
...
def _canonical_descriptor(*, scenario_id: str, observer: str) -> dict[str, Any]:
    ...
    descriptor: dict[str, Any] = {
        ...
        "expected_methods": _core._expected_methods(states),
        "descriptor_sha256": None,
    }
    descriptor["descriptor_sha256"] = _core.compute_fixture_descriptor_sha256(
        descriptor
    )
    return descriptor
    ...
    *,
    ...
    if descriptor["expected_methods"] != canonical["expected_methods"]:
        raise _core._error(
            ...
            "is inconsistent with expected state semantics",
        )
    recorded = _core._hex(
...
def validate_identity(
    ...
    *,
    ...
    expected_specification_sha256 = hashlib.sha256(
        _core._specification_bytes(normalized_descriptor)
    ).hexdigest()
    if specification["sha256"] != expected_specification_sha256:
        raise _core._error(
...
def materialize(document: object, destination: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        ...
        _core._write_bytes(repo / "src" / "access.py", BASE_CODE.encode("utf-8"))
        _core._write_bytes(
...
def verify_materialized(
    ...
    specification_path = repo / str(identity["specification"]["path"])
    try:
        ...
        if specification_sha256 != identity["specification"]["sha256"]:
