"""Fixed wrong-reason fixtures for controlled DW-001 limitations.

The adapter implements a deliberately small owned-synthetic family set whose
source and test bytes are fixed by code. It covers two distinct boundaries:

- a pre-assertion import error that exit-code observation misclassifies as a
  semantic failure while typed observation preserves generic error evidence;
- a genuine assertion failure caused only by collateral behavior while the
  claim-facing assertion is non-discriminating under both observer arms.

No caller-provided executable bytes are accepted. Runtime receipt classes and
independently fixed fixture ground truth remain separate evidence layers.
"""
...
IMPORT_FAMILY_ID = "wrong-reason-base-import-failure"
UNRELATED_ASSERTION_FAMILY_ID = "wrong-reason-unrelated-assertion"
FAMILY_ID = IMPORT_FAMILY_ID
SUPPORTED_FAMILIES = (
    IMPORT_FAMILY_ID,
    UNRELATED_ASSERTION_FAMILY_ID,
)
CONTROL_ROLE = "false-assurance-case"

_IMPORT_CANDIDATE_CODE = """def is_admin(user):
...
_IMPORT_CANDIDATE_TESTS = """import sys
import unittest
...
_UNRELATED_BASE_CODE = """def is_admin(user):
...
_UNRELATED_CANDIDATE_CODE = """def is_admin(user):
    return user.get(\"role\") == \"admin\"
...
_UNRELATED_CANDIDATE_TESTS = """import sys
import unittest

sys.path.insert(0, \"src\")
from access import is_admin, version_label
...
_FAMILY_DEFINITIONS: dict[str, dict[str, object]] = {
    IMPORT_FAMILY_ID: {
        "base_code": _core._BASE_CODE,
        "base_tests": _core._BASE_TESTS,
        "candidate_code": _IMPORT_CANDIDATE_CODE,
        "candidate_tests": _IMPORT_CANDIDATE_TESTS,
        "semantics": {
            "exit-code-v1": {
    ...
    UNRELATED_ASSERTION_FAMILY_ID: {
        "base_code": _UNRELATED_BASE_CODE,
        "base_tests": _core._BASE_TESTS,
        "candidate_code": _UNRELATED_CANDIDATE_CODE,
        "candidate_tests": _UNRELATED_CANDIDATE_TESTS,
        "semantics": {
            "exit-code-v1": {
                "outcomes": ("pass", "fail", "pass", "pass"),
                "causes": (
...
def _definition(family_id: str) -> dict[str, object]:
    try:
        return _FAMILY_DEFINITIONS[family_id]
    except KeyError as exc:
        raise _core._error(
            "fixture descriptor.family_id",
...
def expected_states(
    ...
    *,
    ...
    definition = _definition(family_id)
    semantics = definition["semantics"]
    assert isinstance(semantics, dict)
    try:
        arm = semantics[observer]
    except KeyError as exc:
        raise _core._error(
            "fixture descriptor.observer",
            "is unsupported",
        ) from exc
...
def _canonical_descriptor(
    *,
    ...
    _definition(family_id)
    if observer not in OBSERVER_IDENTIFIERS:
        raise _core._error(
            "fixture descriptor.observer",
            "is unsupported",
        )
    states = expected_states(observer, family_id=family_id)
    descriptor: dict[str, Any] = {
        ...
        "generator": {
            "id": _core.GENERATOR_ID,
            "version": _core.GENERATOR_VERSION,
        },
        "template": {
            "id": _core.TEMPLATE_ID,
            "version": _core.TEMPLATE_VERSION,
        },
        "observer": observer,
    ...
    descriptor["descriptor_sha256"] = (
        _core.compute_fixture_descriptor_sha256(descriptor)
    )
    return descriptor
...
def validate_descriptor(
    ...
    *,
    ...
    if (
        ...
        != _core.FIXTURE_DESCRIPTOR_SCHEMA_VERSION
    ):
        raise _core._error(
            "fixture descriptor.schema_version",
            "is unsupported",
        )
    if descriptor["study_id"] != _core.STUDY_ID:
        raise _core._error(
            "fixture descriptor.study_id",
            "must be 'DW-001'",
        )
    scenario_id = _core._scenario_id(descriptor["scenario_id"])
    family_id = _core._string(
        descriptor["family_id"],
        context="fixture descriptor.family_id",
    )
    _definition(family_id)
    ...
    canonical = _canonical_descriptor(
        scenario_id=scenario_id,
        family_id=family_id,
        observer=observer,
    )
...
def _identity(
    ...
    *,
    ...
    identity["identity_sha256"] = _core.compute_fixture_identity_sha256(
        identity
    )
    return identity
...
def validate_identity(
    ...
    *,
    ...
    normalized_descriptor = validate_descriptor(
        descriptor,
        verify_digest=True,
    )
    document = _core._object(
        identity,
    ...
    if (
        ...
        != _core.FIXTURE_IDENTITY_SCHEMA_VERSION
    ):
        raise _core._error(
            "fixture identity.schema_version",
            "is unsupported",
        )
    for key in (
        "study_id",
...
def materialize(
    ...
    with tempfile.TemporaryDirectory(
        ...
        _core._write_bytes(
            ...
            base_tests.encode("utf-8"),
        )
        _core._write_bytes(
            repo / "deltawitness.toml",
            spec_bytes,
        )
        _core._run_git(
            repo,
        ...
        _core._run_git(
            ...
            "-m",
            (
                "DW-001 synthetic candidate: "
                f"{descriptor['scenario_id']}:{family_id}"
            ),
            timestamp="2001-01-01T00:00:01+00:00",
        )
        head_commit = _core._run_git(
            repo,
            home,
            "rev-parse",
            "HEAD",
        )
        head_tree = _core._run_git(
            repo,
            home,
            "rev-parse",
            "HEAD^{tree}",
...
def verify_materialized(
    ...
    if not repo.is_dir():
        return False, (
            "synthetic fixture destination is not a directory",
        )
    try:
        with tempfile.TemporaryDirectory(
    ...
    if status:
        verification_errors.append(
            "synthetic fixture repository is not clean"
        )
    if head != identity["git"]["head_commit_sha"]:
        verification_errors.append(
            "recorded head commit does not match repository"
        )
    if base_tree != identity["git"]["base_tree_sha"]:
        verification_errors.append(
            "recorded base tree does not match repository"
        )
    if head_tree != identity["git"]["head_tree_sha"]:
        verification_errors.append(
            "recorded head tree does not match repository"
        )
    ...
    try:
        ...
        verification_errors.append(
            "recorded specification cannot be read"
        )
    else:
