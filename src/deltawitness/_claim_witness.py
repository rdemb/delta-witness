"""Internal semantic core for declared logical-test witness localization.

The layer keeps the canonical four-state matrix unchanged. It binds one
operator-declared set of exact standard-library ``unittest`` logical test IDs
to selector-specific observations under only the exact ``BC`` and ``CC``
states from one verified matrix report.

A valid declaration or localization report proves which selectors were
requested and what was observed. It does not prove semantic oracle relevance.
"""
...
_RECEIPT_PRODUCER = "deltawitness-unittest"
...
_OBSERVED = {"pass", "fail", "error", "timeout"}
_AGGREGATE_STATUSES = {
    "supported",
    "unsupported",
    "candidate_invalid",
...
def _validate_declaration(
    ...
    *,
    ...
    expected_commands: list[dict[str, object]] = []
...
def _typed_invalid(exc: BaseException, *, context: str) -> tuple[bool, tuple[str, ...]]:
    if isinstance(exc, ClaimWitnessError):
        return False, (str(exc),)
    return False, (
        f"{context}: verification failed closed: {type(exc).__name__}: {exc}",
    )
...
def verify_claim_witness_declaration_document(
    ...
    try:
        ...
        return _typed_invalid(exc, context="claim witness declaration")
    return True, ()
...
def _preflight_execution(
    ...
    for root_name in ("base_sha", "head_sha"):
        value = _string(
            report[root_name],
            context=f"claim witness source report.{root_name}",
        )
    ...
    for state in _STATE_ORDER:
        commit_sha = _string(
            report["state_commits"][state],
            context=f"claim witness source report.state_commits.{state}",
        )
        tree_sha = _string(
            report["state_trees"][state],
            context=f"claim witness source report.state_trees.{state}",
        )
...
def _selector_classification(
    ...
    incomplete = [
        state for state in states if state["observed"] not in {"pass", "fail"}
    ]
    if incomplete:
        state = incomplete[0]
...
def run_claim_witness_localization(
    ...
    with worktree(
        ...
        for selector, command_entry in zip(
            ...
            states = [_state_document(result) for result in results]
...
def _validate_state(
    ...
    *,
    ...
    observed = _string(state["observed"], context=f"{context}.observed")
    ...
    if receipt_outcome is None:
        ...
        normalized_outcome = _string(
            receipt_outcome,
            context=f"{context}.receipt_outcome",
        )
        normalized_sha = _hex(
            receipt_sha,
        ...
        if producer_name != _RECEIPT_PRODUCER or producer_version != tool_version:
            raise _error(
                f"{context}.receipt producer",
                "does not match unittest-test-id-v1 and report tool version",
            )
        counts = _validate_counts(
            receipt_counts,
            context=f"{context}.receipt_counts",
        )
        if normalized_outcome != "producer_error" and counts["tests_run"] != 1:
            raise _error(
                f"{context}.receipt_counts.tests_run",
                "exact selector evidence must cover exactly one logical test",
            )
        ...
        receipt_outcome = normalized_outcome
    ...
    if timed_out:
        ...
        expected_error = None
    elif receipt_outcome in {"passed", "test_failure"}:
        expected_observed = "error"
        expected_error = "receipt_exit_contradiction"
...
def _validate_source(
    ...
    *,
    ...
    source = _object(value, context="claim witness localization.source")
    _exact_keys(source, _SOURCE_FIELDS, context="claim witness localization.source")
    if source["source_report_schema_version"] != "0.3":
        raise _error(
            "claim witness localization.source.source_report_schema_version",
            "must be '0.3'",
        )
    for field in (
        "declaration_sha256",
        ...
        _hex(
            source[field],
            context=f"claim witness localization.source.{field}",
            lengths=(64,),
        )
    _claim_id(
        source["claim_id"],
        context="claim witness localization.source.claim_id",
    )
...
def _validate_localization(
    ...
    *,
    ...
    aggregate = _string(
        localization["aggregate_status"],
        context="claim witness localization.aggregate_status",
    )
    if aggregate not in _AGGREGATE_STATUSES:
        raise _error(
    ...
    if verify_digests:
        ...
        if computed_semantic != recorded_semantic:
            raise _error(
        ...
        if computed_report != recorded_report:
            raise _error(
                ...
                f"digest mismatch: expected {recorded_report}, "
...
def verify_claim_witness_localization_document(
    ...
    try:
        ...
        return _typed_invalid(exc, context="claim witness source report")
    ...
    try:
        ...
        errors.append(_typed_invalid(exc, context="claim witness localization")[1][0])
