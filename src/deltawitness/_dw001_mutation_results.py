"""Internal execution and verification for frozen claim-scoped mutation results.

This module retains complete unexpected observations as valid development
results. Preregistered expectations remain immutable and are stored separately
from observed outcomes; concordance is derived rather than used as a validity
condition. Malformed or relationally inconsistent evidence still fails closed.
"""
...
def expected_observation(
    *,
    ...
    """Build one canonical complete expected observation."""
...
def execute_observation(
    *,
    ...
    return {
        ...
        "duration_seconds": getattr(process, "duration_seconds"),
        "stdout_sha256": getattr(process, "stdout_sha256"),
        "stderr_sha256": getattr(process, "stderr_sha256"),
        "invocation_binding": binding,
        "receipt_sha256": getattr(process, "receipt_sha256"),
        "receipt_outcome": getattr(process, "receipt_outcome"),
        "receipt_producer": getattr(process, "receipt_producer"),
        "receipt_counts": getattr(process, "receipt_counts"),
...
def _derive_summary(
    ...
    generic = [record for record in records if record["record_role"] == "generic_operator"]
    historical = [
        record
        for record in records
        if record["record_role"] == "historical_challenge_control"
    ]
    generation = [record for record in records if record["record_role"] == "generation_control"]

    def profile_outcome(record: Mapping[str, object], profile_id: str) -> str:
    ...
    candidate_valid = (
        ...
        and all(profile["outcome"] == "baseline_passed" for profile in candidate_profiles)
        and isinstance(candidate_reference, dict)
        and candidate_reference["outcome"] == "reference_passed"
    )
    return {
        ...
        "generic_strong_killed": sum(
            profile_outcome(record, _STRONG_PROFILE_ID) == "killed"
            for record in generic
        ),
        "generic_strong_survived": sum(
            profile_outcome(record, _STRONG_PROFILE_ID) == "survived"
            for record in generic
        ),
        "generic_strong_indeterminate": sum(
            profile_outcome(record, _STRONG_PROFILE_ID) == "indeterminate"
            for record in generic
        ),
        "generic_weak_killed": sum(
            profile_outcome(record, _WEAK_PROFILE_ID) == "killed"
            for record in generic
        ),
        "generic_weak_survived": sum(
            profile_outcome(record, _WEAK_PROFILE_ID) == "survived"
            for record in generic
        ),
        "generic_weak_indeterminate": sum(
            profile_outcome(record, _WEAK_PROFILE_ID) == "indeterminate"
            for record in generic
        ),
        "generic_claim_violations_observed": sum(
            isinstance(record["reference"], dict)
            and record["reference"]["outcome"] == "claim_violation_observed"
            for record in generic
        ),
        "mutation_score": None,
    }
...
def _derive_analysis(
    ...
    implementations = [candidate, *records]
    ...
    for record in implementations:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                if profile["concordant"] is not True:
                    unexpected_profiles += 1
                selectors = profile["selectors"]
                if isinstance(selectors, list):
                    unexpected_observations += sum(
                        selector["concordant"] is not True
                        for selector in selectors
                    )
        reference = record["reference"]
        if isinstance(reference, dict):
            if reference["concordant"] is not True:
        ...
        if record["concordant"] is not True:
            unexpected_record_ids.append(str(record["record_id"]))
    return {
        "status": "expected" if not unexpected_record_ids else "unexpected",
        "candidate_baseline_concordant": candidate["concordant"] is True,
    ...
    with tempfile.TemporaryDirectory(
        ...
        if not isinstance(expected_reference, dict):
            raise _error(
                ...
                "expected reference must be an object",
            )
        reference_selectors = [
            _enrich_observation(
                executor(
        ...
        reference_outcome = _reference_outcome(
            [item["observed"] for item in reference_selectors]
        )
        actual_reference = {
            "profile_id": _REFERENCE_PROFILE_ID,
            ...
            "concordant": (
                reference_outcome == expected_reference["expected_outcome"]
            ),
        }
...
def _preflight(
    ...
    if plan["execution_authorized"] is not False:
        raise _error(
            ...
            "must remain false; execution is authorized only by this separately reviewed fixed runner",
        )
    return plan, catalog
...
def run_result(
    ...
    *,
    ...
    """Execute the frozen catalog and retain expected or unexpected evidence."""
    ...
    for template in record_templates:
        ...
        if template["record_role"] == "historical_challenge_control":
            ...
            frozen_record = catalog_by_id[str(template["mutant_id"])]
    ...
    result["semantic_sha256"] = compute_semantic_sha256(result)
    result["report_sha256"] = compute_report_sha256(result)
    valid, errors = verify_result(result, normalized_plan, normalized_catalog)
    if not valid:
        raise _error(
...
def _canonical_observation(
    ...
    observation = _exact_keys(
        ...
        _OBSERVATION_FIELDS,
        context=(
            f"claim-scoped mutation result observation "
    ...
    if observed in {"pass", "fail"}:
        canonical_complete = expected_observation(
            plan_sha256=str(expected["invocation_binding_plan_sha256"]),
            catalog_sha256=str(expected["invocation_binding_catalog_sha256"]),
        ...
        for field in (
            ...
            if observation[field] != canonical_complete[field]:
                raise _error(
                    f"claim-scoped mutation result observation.{field}",
                    "is inconsistent with typed pass/fail evidence",
                )
    else:
        if observed == "timeout" and observation["timed_out"] is not True:
            raise _error(
                "claim-scoped mutation result observation.timed_out",
                "must be true for timeout",
            )
        if observed == "error" and observation["timed_out"] is True:
            raise _error(
    ...
    canonical["expected_observed"] = expected["expected_observed"]
    canonical["concordant"] = observed == expected["expected_observed"]
    return canonical


def _observation_expectation_context(
    expected: Mapping[str, object],
    *,
    plan: Mapping[str, object],
    catalog: Mapping[str, object],
    ...
    context["invocation_binding_plan_sha256"] = plan["plan_sha256"]
    context["invocation_binding_catalog_sha256"] = catalog["catalog_sha256"]
    return context
...
def _canonical_profile(
    ...
    *,
    ...
    for field in ("profile_id", "profile_role"):
    ...
    selectors = [
        _canonical_observation(
            actual_selector,
            _observation_expectation_context(
                expected_selector,
                plan=plan,
                catalog=catalog,
            ),
    ...
    outcome = _profile_outcome(
        [selector["observed"] for selector in selectors],
        baseline=baseline,
    )
    return {
...
def _canonical_reference(
    ...
    *,
    ...
    if reference["profile_id"] != _REFERENCE_PROFILE_ID:
        raise _error(
            "claim-scoped mutation result reference.profile_id",
            "is unsupported",
        )
...
def _canonical_result(
    ...
    base = _base_result(plan, catalog, candidate, records)

    created_at = result["created_at"]
    if not isinstance(created_at, str) or not created_at:
        raise _error(
            "claim-scoped mutation result.created_at",
    ...
    if any(not isinstance(value, str) or not value for value in runtime.values()):
        raise _error(
            "claim-scoped mutation result.runtime",
    ...
    if (
        cost["status"] != "measured"
        or cost["implementation_count"] != 5
        or cost["command_count"] != 25
        or cost["selector_count"] != 25
        or cost["missing_reason"] is not None
    ):
        raise _error(
    ...
    canonical["runtime"] = deepcopy(runtime)
...
def verify_result(
    ...
    try:
        ...
        computed_semantic = compute_semantic_sha256(document)
        ...
        computed_report = compute_report_sha256(document)
