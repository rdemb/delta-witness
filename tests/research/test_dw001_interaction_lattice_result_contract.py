...
class DW001InteractionLatticeResultContractTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        except Exception as exc:  # preserve one root failure for every contract
    ...
    def test_result_binds_exact_merged_preregistration_and_execution_protocol(self) -> None:
        ...
        self.assertEqual(
            result["pr46_result_report_sha256"],
            self.pr46_result["report_sha256"],
        )
    ...
    def test_per_quadrant_statement_arc_branch_and_path_shapes_are_exact(self) -> None:
        ...
        for selector in result["candidate_selectors"]:
            ...
            self.assertIsNone(branch["missing_branch_arcs"])
            self.assertEqual(
    ...
    def test_path_multiset_is_order_independent_but_preserves_multiplicity(self) -> None:
        result = self._result()
        records = result["profiles"][1]["path_records"]
    ...
    def test_complete_preregistration_divergence_is_retained_as_unexpected(self) -> None:
        ...
        def divergent_selector(**kwargs):
            ...
            executable = receipt["statement_evidence"]["executable"]
            target_lines = receipt["target"]["target_lines"]
            receipt["statement_evidence"] = {
                "executable": executable,
                "executed": [2],
                "missing": sorted(set(executable) - {2}),
                "measured_lines": [2],
                ...
                "target_missing": sorted(
                    (set(executable) & set(target_lines)) - {2}
                ),
            }
